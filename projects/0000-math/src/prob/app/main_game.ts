...
export class MainGame implements IProbIF {
  ...
  spin(param: {
    ...
    // 上好路的押注判斷
    if (machineData.betList && machineData.betList.length > 0) {
      betList = machineData.betList;
    }
    ...
    if (gameState.nextState === status.NormalGame) {
      ...
      // 假如有虛寶卡要判斷是否符合
        ...
        if (Object.values(betList).indexOf(forceBingoCardBet) === -1) {
          log.emerg(
    ...
    if (result.gameState.state === status.NormalGame) {
      ...
      let gameInfoObj = {
        ...
        accTotalWin: gameState.roundTotalwin, // 累積贏分
